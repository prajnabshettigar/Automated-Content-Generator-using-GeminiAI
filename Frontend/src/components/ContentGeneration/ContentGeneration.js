...
import { generatecontentAPI } from "../../apis/chatgenrator/chatgen";
...
const AIAssistant = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      const stringifiedData = JSON.stringify(data); // Convert to string
      setGeneratedContent(stringifiedData);
    },
  });
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      // Prepare the prompt and trigger the mutation
      const promptText = `Generate a blog post based on ${values.prompt}, ${values.category}, ${values.tone}`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-900 flex justify-center items-center p-6">
      <div className="bg-white rounded-xl shadow-xl overflow-hidden max-w-2xl w-full p-6">
        ...
        <div className="flex mt-3">
          ...
          <div className="mr-2 mb-2">
            <span className="text-sm font-semibold bg-green-200 px-4 py-2 rounded-full">
              Credit: {data?.user?.apireq_count} / {data?.user?.min_req_count}
            </span>
          </div>
        </div>
